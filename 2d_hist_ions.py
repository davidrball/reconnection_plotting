...
savename = "bguide.3_triggered_ions"
tfinal = 65
gamthresh=1 #don't include cold thermals
...
gammae_final = np.array(myf_final['gammai'])
...
wpar_final = np.abs(np.array(myf_final['edotvi']))[myind]*2/.45
...
plt.hist2d(np.log10(np.abs(wpar_final)), np.log10(gammae_final),bins=(wparbins,gambins),norm=LogNorm())
#plt.xscale('log')
#plt.yscale('log')
#plt.xlim(1e2,2e3)
...
plt.savefig(savename+'_wparz_gam_t'+str(tfinal)+'.png',dpi=300,bbox_inches='tight')
plt.close()
