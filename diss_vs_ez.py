...
for t in range(tstart,tfinal):
    ...
    for i in range(xlen):
        for j in range(ylen):
            ...
            D[i,j]=tmp4
    D *= kene #comment out if we don't want to include relativistic correction (scaling by gamma)
    ...
    cbar_ax = fig.add_axes([.125, .93, .35, .03])
    cbar_ax2 = fig.add_axes([.55,.93, .35, .03])

    im1 = ax1.imshow(D[ylow:yup,xlow:xup],origin='lower',vmin=0,vmax=1)
    #plt.colorbar(label='$D_{e}$')
    #plt.savefig('diss_test.png')
    im2 = ax2.imshow(ez_bxy[ylow:yup,xlow:xup],origin='lower',vmin=0,vmax=1)
    cb = fig.colorbar(im1, cax=cbar_ax,orientation="horizontal")
    cbar_ax.set_xlabel('$D_{e}$')
    cbar_ax.xaxis.set_label_position('top')

    cb2 = fig.colorbar(im2, cax=cbar_ax2,orientation="horizontal")
    cbar_ax2.set_xlabel('$E_{z}/B_{xy}$')
    cbar_ax2.xaxis.set_label_position('top')
    #ax3.imshow(dense, origin='lower',vmin=0,vmax=10)
    #ax3.set_title('Electron Density')

    plt.savefig('diss_plots/'+t0_str+'.png',dpi=300,bbox_inches='tight')
    plt.close()
    f0.close()
    fparam.close()
