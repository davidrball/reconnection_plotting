...
fld_base1 = "../../tristan-mp_reconnection/guide_fields/sig.3/simulation_matrix/delgam05/bguide.5/output/flds.tot.0"
fld_base2 = "../../tristan-mp_reconnection/guide_fields/sig.3/simulation_matrix/delgam05/bguide.7/output/flds.tot.0"
fld_base3 = "../../tristan-mp_reconnection/guide_fields/sig.3/simulation_matrix/delgam05/bguide1/output/flds.tot.0"
...
'''
#accmec simiulations
fld_base0 = "../../tristan_acc-mec_Ez/8k_bguide0_triggered_stride1_thresh2/output/flds.tot.0"
fld_base1 = "../../tristan_acc-mec_Ez/8k_bguide0_untriggered_stride1_thresh2/output/flds.tot.0"
fld_base2 = "../../tristan_acc-mec_Ez/8k_bguide.3_triggered_stride1_thresh2/output/flds.tot.0"
fld_base3 = "../../tristan_acc-mec_Ez/8k_untriggered_bguide.3/output/flds.tot.0"
'''
...
for t in range(tstart,tfinal):
    ...
    btot0 = np.rot90(btot0[:,x0low:x0up])
    btot1 = np.rot90(btot1[:,x1low:x1up])
    btot2 = np.rot90(btot2[:,x2low:x2up])
    btot3 = np.rot90(btot3[:,x3low:x3up])
    ...
    myvmin = 0
    myvmax = 4
    im0 = ax0.imshow(btot0/4.,vmin=myvmin,vmax=myvmax, extent = [-yext/2,yext/2,-xext/2,xext/2],origin='lower')
    im1 = ax1.imshow(btot1/4.,vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2], origin='lower')
    im2 = ax2.imshow(btot2/4.,vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2],origin='lower')
    im3 = ax3.imshow(btot3/4.,vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2],origin='lower')
#linear scale
...
#ax1.set_yticks([-1,-.5,0,.5,1])
    ...
    ax1.annotate("$B_{g}/B_{0}=0.5$",(-.4,.25),color="White",size=14,fontweight='bold')    
...
#fig.colorbar(im3, orientation="horizontal",cax=cbar_ax,label="Density (particles per cell)")
    ...
    cbar_ax.set_xlabel("Density ($ppc/ppc_{0}$)",fontsize=14, labelpad=-45)
...
#cb.ax.set_xticklabels([0,4,8,12,16,20])
    ...
    fig.savefig('fourdens_plots/highbeta/otherguide/'+t_str+'.png',dpi=300,bbox_inches='tight')
    '''
    plt.figure()
    ax = plt.gca()
