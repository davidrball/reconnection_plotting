...
for t in range(tstart,tfinal):
    ...
    #calculating these:
    va0 = np.rot90(vy0/np.sqrt(sigfinal0 / (1+sigfinal0)))
    va1 = np.rot90(vy1/np.sqrt(sigfinal1 / (1+sigfinal1)))
    va2 = np.rot90(vy2/np.sqrt(sigfinal2 / (1+sigfinal2)))
    va3 = np.rot90(vy3/np.sqrt(sigfinal3 / (1+sigfinal3)))
...
#fig.colorbar(im3, orientation="horizontal",cax=cbar_ax,label="Density (particles per cell)")
    ...
    cbar_ax.set_xlabel("$|\Delta v_{x}|/v_{A}$",fontsize=14, labelpad=-45)
...
#cb.ax.set_xticklabels([0,4,8,12,16,20])
    ...
    fig.savefig('va_test.png',dpi=300,bbox_inches='tight')
