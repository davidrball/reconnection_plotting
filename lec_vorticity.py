...
for t in range(tstart,tfinal):
    ...
    vx0 = (mi+me)*f0['v3x'][0,:,:] - mi*f0['v3xi'][0,:,:]
    vx1 = (mi+me)*f1['v3x'][0,:,:] - mi*f1['v3xi'][0,:,:]
    vx2 = (mi+me)*f2['v3x'][0,:,:] - mi*f2['v3xi'][0,:,:]
    vx3 = (mi+me)*f3['v3x'][0,:,:] - mi*f3['v3xi'][0,:,:]

    xscan = 100
    xhlf = np.shape(vy0)[1]/2
    xup = int(xhlf + xscan)
    xlow = int(xhlf-xscan)
    ...
    #remember, vx is across sheet (inflow)
    #vy is along sheet (outflow)
    ...
    logvort0 = np.log10(np.abs(vort0))
    logvort1 = np.log10(np.abs(vort1))
    logvort2 = np.log10(np.abs(vort2))
    logvort3 = np.log10(np.abs(vort3))

    myvmin = -1
    myvmax = 0

    im0 = ax0.imshow(np.rot90(logvort0),vmin=myvmin,vmax=myvmax, extent = [-yext/2,yext/2,-xext/2,xext/2],origin='lower')
    im1 = ax1.imshow(np.rot90(logvort1),vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2], origin='lower')
    im2 = ax2.imshow(np.rot90(logvort2),vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2],origin='lower')
    im3 = ax3.imshow(np.rot90(logvort3),vmin=myvmin,vmax=myvmax,extent=[-yext/2,yext/2,-xext/2,xext/2],origin='lower')
#linear scale
...
#fig.colorbar(im3, orientation="horizontal",cax=cbar_ax,label="Density (particles per cell)")
    ...
    cbar_ax.set_xlabel("$\log|\\nabla \\times v_{e}|_{z}$",fontsize=14, labelpad=-45)
