...
t_final=40 #corresponds to .8 of an alfven crossing time
...
#fld_base = "../../tristan_acc-mec_Ez/testing_edotv/bguide.1/output/flds.tot."
...
#max_E_ind = np.argmax(gammae) # 2nd most energetic particle
    ...
    EoverB_list = []
    wpar_list = []
    for t in range(1,40,1): #in some cases the high E particle is injected later in time, so it'
        t_list.append(t)    
        t_string = '%03d' % t
    ...
    #beginning of new stuff to handle processor index as well
        ...
        #print(i)
            if tmp[i] == 0 and tmp_proc[i]==0:
                ...
                tmpwpar = np.array(f_prtl['edotve'])[tmp_ind]
            ...
            #plt.savefig("/home/dball/tristan_out/real_runs/movie_plots/particle_tracking/untriggered/betap1/E"+str(int(part_num)) + "_particle/"+t_string+".png",bbox_inches='tight',dpi=400)
                tstr_save = str(t)
                plt.savefig('lowE_singleprt/E'+ str(part_num)+'_' +tstr_save+'.png',bbox_inches='tight',dpi=300)
                plt.close()
                f_fld.close()
                f_prtl.close()
                break
