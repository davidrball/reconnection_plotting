...
beta_list = [.0003, .003, .03, .25, .0003, .003, .03 ,.25, .0003, .003, .03, .25, .0003, .003, .03, .25]
delgam_list = [.00005, .0005, .005, .05, .00005, .0005, .005, .05, .00005, .0005,.005, .05, .00005, .0005, .005, .05]
...
for i in range(len(matrix_namelist)):
    for j in range(len(matrix_namelist[0])): #assuming that the matrix is square 
        if matrix_namelist[i][j] == "pass":
            ...
            myax_ind = 4*i + j
            mybeta = beta_list[myax_ind]
            mytheta = delgam_list[myax_ind]

            beta = beta_slice(matrix_namelist[i][j], mybeta, mytheta, 4, 200)

            myax = axlist[myax_ind]
            title = str(betaeff_list[myax_ind])
            myax.imshow(np.log10(np.rot90(beta)),vmin=-2,vmax=2,origin='lower')
...
plt.savefig('4x4_beta_t35.png',dpi=300,bbox_inches='tight')
