...
runstr = "bguide.1_triggered_injhist"
tfinal = 60
gamthresh=300
tcs_offset = 5


savename = runstr+"_offset"+str(tcs_offset)+ "_gamthresh"+str(gamthresh)+"_tfinal"+str(tfinal)

filename = "wpar_gam_txt/"+savename+'.txt'

#prtl_base = "/home/u21/davidrball/david/tristan_acc-mec_Ez/bguide.3_wpar_zcomp/gamprt_thresh50/output/prtl.tot."
prtl_base = "/home/u21/davidrball/david/tristan_acc-mec_Ez/bguide.1_triggered_stride1_gamthresh50/output/prtl.tot."
...
print('writing two lists of length', len(wpar_plot_list))

for i in range(len(wpar_plot_list)):
    target.write(str(wpar_plot_list[i]))
    target.write(' , ')
target.write('\n')
for i in range(len(wpar_plot_list)):
    target.write(str(gam_plot_list[i]))
    target.write(' , ')
target.close()
